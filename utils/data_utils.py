...
def exp_dir_setup(args):
    ...
    # Add seed to the directory path if it exists
    if hasattr(args, 'random_seed'):
        output_dir += '/seed_{}'.format(args.random_seed)
        yaml_config['random_seed'] = args.random_seed
    ...
    if args.debug:
        ...
        if os.path.exists(output_dir + '/config.yaml'):
            exp_yaml = load_yaml(output_dir + '/config.yaml')
            if exp_yaml == yaml_config:
                return yaml_config, output_dir, 0
